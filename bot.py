...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cur.fetchone():
        ...
        cur.execute(
            "INSERT INTO users (id, username, ref_by) VALUES (?,?,?)",
            (user.id, user.username, ref_by)
        )
        ...
        if ref_by:
            ...
            try:
                await context.bot.send_message(
                    ref_by,
                    f"🎉 Referral! +1000 KHOR. New user: @{user.username}"
                )
            except:
                pass
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "submit":
        ...
        cur.execute(
            "SELECT balance, wallet FROM users WHERE id=?",
            (uid,)
        )
        ...
        cur.execute(
            "SELECT COUNT(*) FROM users WHERE ref_by=?",
            (uid,)
        )
...
async def save_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ *Wallet Saved!*\n"
        f"`{txt}`\n\n"
        f"Airdrop ke time distribution hoga.",
        parse_mode="Markdown"
    )
...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute(
        "SELECT COUNT(*) FROM users WHERE wallet IS NOT NULL"
    )

    with_wallet = cur.fetchone()[0]

    await update.message.reply_text(
        f"📊 *Admin Panel*\n"
        f"Total: {total}\n"
        f"With Wallet: {with_wallet}\n\n"
...
async def export_csv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute(
        "SELECT id, username, wallet, ref_by, balance "
        "FROM users WHERE wallet IS NOT NULL"
    )
    ...
    await update.message.reply_document(
        open("khor_wallets.csv", "rb"),
        caption=f"📁 {len(rows)} Wallets"
    )
